...
class MenuHighlighter {
  ...
  updateActiveMenu() {
    ...
    const submenuLink = document.querySelector(
      `.submenu a[data-section="${sectionId}"]`,
    );
    if (submenuLink) {
    ...
    if (parentMenuId) {
      const parentMenuItem = document.querySelector(
        `li[data-menu="${parentMenuId}"] > .nav-item`,
      );
      if (parentMenuItem) {
  ...
  clearAllActiveStates() {
    ...
    document
      .querySelectorAll(".nav-item.active, .nav-item.parent-active")
      .forEach((item) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const checkNavigationSystem = setInterval(() => {
    if (window.navigation && window.navigation.navigateToSection) {
      ...
      const originalNavigate = window.navigation.navigateToSection.bind(
        window.navigation,
      );

      // Override with patched version
      window.navigation.navigateToSection = async function (
        sectionId,
        sectionName,
        state = {},
      ) {
