  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({
        success: false,
        error:
          "Payroll period not found. Please initialize BT05 in py_stdrate.",
      });
    }
    ...
    res.json({
      ...
      blockReason: canProcessMonthEnd
        ? null
        : `Month-end can only be processed when status is 999 (Calculation Completed). Current status: ${period.status} - ${statusMessage}`,
    });
  } catch (error) {
...
router.post("/process-monthend", verifyToken, async (req, res) => {
  ...
  try {
    ...
    console.log(
      `🔄 Starting month-end processing: ${currentPeriod.year}-${currentPeriod.month}`,
    );
    console.log(`   Database: ${currentDb}`);
    console.log(`   User: ${userFullname} (${userId})`);
    console.log(
      `   Current Status: ${currentPeriod.status} - ${getStatusMessage(currentPeriod.status)}`,
    );
    ...
    try {
      ...
      await connection.query(`CALL py_py37Monthend(?, ?, ?)`, [
        currentPeriod.month,
        currentPeriod.year,
        userId,
      ]);
      ...
      const employeesProcessed =
        extractRecLog.length > 0 ? extractRecLog[0].records_processed : 0;
      const recordsProcessed =
        monthendLog.length > 0 ? monthendLog[0].records_processed : 0;
      const executionTime =
        monthendLog.length > 0 ? monthendLog[0].execution_time_ms : 0;

      console.log(
        `📊 Statistics: Employees=${employeesProcessed}, Records=${recordsProcessed}`,
      );
      ...
      console.log(
        `📅 Moving period: ${currentPeriod.month}/${currentPeriod.year} → ${nextMonth}/${nextYear}`,
      );
      ...
      console.log(
        "✅ BT05 status updated: sun=0, sat=0 (Data Entry Open), period advanced",
      );
      ...
      console.error(
        "❌ Month-end processing failed, transaction rolled back:",
        procError,
      );
      throw procError;
    }
  ...
  try {
    ...
    if (companyRows.length === 0) {
      return res.status(400).json({
        success: false,
        error:
          "No company found. Year-end processing requires a valid company.",
      });
    }
    ...
    console.log(
      "✅ Validation passed: Month=1, Previous Month=12, Year matches",
    );
    ...
    try {
      ...
      console.log(
        `✅ Updated year from ${currentPeriod.year} to ${currentPeriod.year + 1}`,
      );
      ...
      if (isNavyCompany) {
        ...
        console.log(
          `ℹ️  Skipping emolumentform update (Company: ${companyName}, not NAVY)`,
        );
      }
      ...
      await connection.rollback();
      console.error(
        "❌ Year-end processing failed, transaction rolled back:",
        procError,
      );
      throw procError;
    }
