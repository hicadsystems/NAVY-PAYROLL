...
(function () {
  ...
  const personnelProfileMenu = document.querySelector(
    'li[data-menu="personel-profile"]'
  );
  ...
  if (!personnelProfileMenu) {
    console.warn(
      "Personnel Profile menu (personel-profile) not found. Only Data Input visibility will be managed."
    );
  }
  ...
  async function loadPayrollStatus() {
    try {
      ...
      const res = await auth.fetchWithAuth("/status-payroll", {
        method: "GET",
        headers: {
  ...
  function updateMenuVisibility() {
    ...
    if (shouldHide) {
      console.log(
        `✓ Menus **hidden** - Payroll files saved (stage: ${currentPayrollStage})`
      );
    } else {
      console.log(
        `✓ Menus **visible** - Data entry reopened (stage: ${currentPayrollStage})`
      );
    }
  }
