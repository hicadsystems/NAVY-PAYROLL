...
} = require("../../middware/hierarchyGuard");
...
async function discoverMigrationTables(connection, database) {
  try {
    ...
    if (tables.length === 0) {
      ...
      console.log(
        `  ⚠️ Sample tables in ${database}:`,
        allTables.map((t) => t.table_name).join(", "),
      );
    }
    ...
    for (const { table_name } of tables) {
      ...
      if (columns.length > 0) {
        ...
        console.log(
          `    ⚠️ ${table_name} has no matching employee ID column. Sample columns:`,
    ...
    console.error(
      `  ❌ Error discovering migration tables in ${database}:`,
      error.message,
    );
    return [];
  }
...
async function discoverOrGetDefaultMigrationTables(connection, database) {
  ...
  console.log(
    `  ✓ Found ${existingTables.length} existing tables from default list`,
  );
...
router.get("/active-employees", verifyToken, async (req, res) => {
  try {
    ...
    // User may only list employees in their own class or below
...
router.post("/payroll-class", verifyToken, async (req, res) => {
  ...
  if (!PayrollClass || PayrollClass.trim() === "") {
    return res
      .status(400)
      .json({ success: false, error: "Payroll class is required" });
  }
  ...
  try {
    ...
    console.log(
      `   Current Payroll Class: ${currentPayrollClass || "Not Assigned"}`,
    );
    ...
    if (
      ...
      console.log(
        `   Source DB: ${sourceDb} (from payrollclass ${currentPayrollClass})`,
      );
    }
    ...
    if (!targetExists) {
      return res.status(400).json({
        ...
        action:
          "Please create the database or update your payroll class configuration.",
        debug: {
    ...
    if (
      ...
      console.log(
        `Employee in OFFICERS database has no payroll class. Assigning class '${payrollClassInput}'...`,
      );
      ...
      if (payrollClassCheck.length === 0) {
        ...
        console.log(
          `✓ Created payroll class '${payrollClassInput}' in py_payrollclass`,
        );
      }
      ...
      console.log(
        `✅ Assigned payroll class '${payrollClassInput}' to employee ${employeeId}`,
      );
    ...
    // Hierarchy check: personnel can only be moved upward (to a lower classcode / higher rank).
    // Also verifies the user has access to the source class.
    if (!(await assertUpwardMigrationOnly(req, res, sourceDb, targetDb)))
      return;
    ...
    try {
      ...
      throw new Error(
        `Source database "${sourceDb}" does not exist or is not accessible.`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Target database "${targetDb}" (${targetName}) does not exist. Please create it first.`,
      );
    }
    ...
    await sourceConnection.query(`USE \`${sourceDb}\``);
    const sourceStandardized = await standardizeEmployeeIdColumns(
      sourceConnection,
      sourceDb,
      standardTables,
    );
    if (sourceStandardized.length > 0) {
      console.log(
        `  ✓ Standardized ${sourceStandardized.length} tables in source database`,
      );
    }

    await targetConnection.query(`USE \`${targetDb}\``);
    const targetStandardized = await standardizeEmployeeIdColumns(
      targetConnection,
      targetDb,
      standardTables,
    );
    if (targetStandardized.length > 0) {
      console.log(
        `  ✓ Standardized ${targetStandardized.length} tables in target database`,
      );
    }
    ...
    console.log(
      `Discovering migration tables in source and target databases...`,
    );
    await sourceConnection.query(`USE \`${sourceDb}\``);
    const sourceMigrationTables = await discoverOrGetDefaultMigrationTables(
      sourceConnection,
      sourceDb,
    );

    await targetConnection.query(`USE \`${targetDb}\``);
    const targetMigrationTables = await discoverOrGetDefaultMigrationTables(
      targetConnection,
      targetDb,
    );
    ...
    const migrationTables = sourceMigrationTables.filter((sourceTable) =>
      targetMigrationTables.some(
        (targetTable) => targetTable.table === sourceTable.table,
      ),
    );
    ...
    if (migrationTables.length > 0) {
      console.log(
        `Tables: ${migrationTables.map((t) => `${t.table}(${t.emplIdCol})`).join(", ")}`,
      );
    }
    ...
    for (const { table, emplIdCol } of migrationTables) {
      try {
        const actualColName = await getActualColumnName(
          targetConnection,
          targetDb,
          table,
          emplIdCol,
    ...
    if (existsInTarget) {
      ...
      for (const table in recordsToDelete) {
        ...
        try {
          ...
          console.log(
            `  ✓ Deleted ${result.affectedRows} record(s) from ${table}`,
          );
        } catch (err) {
    ...
    for (const { table, emplIdCol } of migrationTables) {
      try {
        ...
        if (sourceTableExists[0].count > 0 && targetTableExists[0].count > 0) {
          ...
          const sourceColName =
            sourceColumns.length > 0 ? sourceColumns[0].COLUMN_NAME : emplIdCol;
          ...
          if (records.length > 0) {
            ...
            console.log(
              `  ✓ Migrated ${records.length} record(s) from ${table}`,
            );
          }
        }
      } catch (err) {
...
router.post("/payroll-class/bulk", verifyToken, async (req, res) => {
  ...
  if (!TargetPayrollClass || TargetPayrollClass.trim() === "") {
    return res
      .status(400)
      .json({ success: false, error: "Target payroll class is required" });
  }
  ...
  // Hierarchy check: bulk migration upward only
  if (!(await assertUpwardMigrationOnly(req, res, sourceDb, targetDb))) return;
  ...
  try {
    ...
    const unionQueries = migrationTables
      .map(
        ({ table, emplIdCol }) =>
        ...
        for (const { table, emplIdCol } of migrationTables) {
          try {
            ...
            if (tableExists[0].count > 0) {
              ...
              const actualColName =
                columns.length > 0 ? columns[0].COLUMN_NAME : emplIdCol;
...
router.post("/payroll-class/range", verifyToken, async (req, res) => {
  ...
  if (!StartEmpl_ID || !EndEmpl_ID || !TargetPayrollClass) {
    return res.status(400).json({
      success: false,
      error:
        "Start employee ID, end employee ID, and target payroll class are required",
    });
  }
  ...
  // Hierarchy check: range migration upward only
  if (!(await assertUpwardMigrationOnly(req, res, sourceDb, targetDb))) return;
...
router.get("/payroll-class/preview/:Empl_ID", verifyToken, async (req, res) => {
  ...
  if (!PayrollClass) {
    return res
      .status(400)
      .json({ error: "PayrollClass query parameter required" });
  }
  ...
  try {
    ...
    res.json({
      ...
      warning:
        "This operation will move all employee data to the new database and delete from current database. This cannot be undone.",
    });
  } catch (error) {
    console.error("❌ Preview error:", error);
    res
      .status(500)
      .json({ error: "Failed to generate preview", details: error.message });
  }
});
