...
class UserNotificationSystem {
  ...
  setupCrossTabSync() {
    window.addEventListener("storage", (e) => {
      ...
      if (e.key === this.storageKey && e.newValue) {
        console.log(
          `📩 [${this.tabId}] Received notification update from another tab`
        );
      ...
      if (["POST", "PUT", "DELETE"].includes(method)) {
        ...
        if (existingLock) {
          ...
          if (Date.now() - lockData.timestamp < 2000) {
            console.log(
              `🔒 [${self.tabId}] Another tab is processing, skipping`
            );
            return response;
          }
        }
  ...
  loadUserNotifications() {
    ...
    if (stored) {
      try {
        ...
        console.log(
          `📂 [${this.tabId}] Loaded ${this.notifications.length} notifications from localStorage`
        );
      } catch (e) {
  ...
  saveUserNotifications() {
    try {
      ...
      console.log(
        `💾 [${this.tabId}] Saved ${this.notifications.length} notifications to localStorage`
      );
    } catch (e) {
  ...
  async syncWithBackend() {
    try {
      ...
      const response = await window.authService.fetchWithAuth(
        "/notifications",
        {
          method: "GET",
          headers: {
      ...
      if (
        ...
        if (newCount > 0) {
          console.log(
            `[${this.tabId}] Synced ${newCount} new notifications from backend`
          );
          this.render();
        } else {
          console.log(
            `[${this.tabId}] Backend sync complete - no new notifications`
          );
        }
  ...
  saveEvent(event) {
    ...
    if (exists) {
      console.log(
        `⏭️ [${this.tabId}] Event already saved, skipping:`,
        event.id
      );
      return;
    }
  ...
  animateRecentEvents() {
    ...
    console.log(
      `🎬 [${this.tabId}] Animating ${recentEvents.length} recent events`
    );
  ...
  animateEventDrop(event) {
    ...
    if (!container) {
      console.log(
        `⏭️ [${this.tabId}] Not on dashboard, adding directly to panel`
      );
    ...
    floatingNotif.className =
      "fixed top-32 left-1/2 transform -translate-x-1/2 bg-white shadow-xl rounded-lg p-4 border-l-4 z-50 min-w-[280px] opacity-0";
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        floatingNotif.style.transition =
    ...
    setTimeout(() => {
      ...
      console.log(
        `[${this.tabId}] Animation complete, adding to panel:`,
        event.message
      );
  ...
  addToPanel(notification) {
    ...
    if (this.animatingIds.has(notification.id)) {
      console.log(
        `⏭️ [${this.tabId}] Notification still animating, skipping:`,
        notification.id
      );
      return;
    }
    ...
    if (exists) {
      console.log(
        `⏭️ [${this.tabId}] Notification already exists, skipping:`,
        notification.id
      );
      return;
    }
    ...
    const existingIndex = this.notifications.findIndex(
      (n) =>
        n.message === notification.message && Date.now() - n.timestamp < 60000
    );
    ...
    if (existingIndex !== -1) {
      ...
      console.log(
        `🔄 [${this.tabId}] Duplicate detected, count now: ${existing.count}`
      );
    ...
    console.log(
      `➕ [${this.tabId}] Adding new notification:`,
  ...
  render() {
    ...
    const html = displayNotifs
      .map((notif, index) => {
        const icon = this.getIcon(notif.type);
        const timeAgo = this.getTimeAgo(notif.timestamp);
        const countBadge =
          notif.count > 1
            ? `<span class="ml-2 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-blue-500 rounded-full">${notif.count}x</span>`
            : "";
    ...
    asideLinks.forEach((link) => {
      ...
      if (linkText.includes("notification")) {
        ...
        if (count > 0) {
          ...
          badge.className =
            "notification-count-badge ml-2 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-red-500 rounded-full";
  ...
  renderAllNotifications() {
    ...
    const allNotifsHtml = this.notifications
      .map((notif) => {
        const icon = this.getIcon(notif.type);
        const timeAgo = this.getTimeAgo(notif.timestamp);
        const countBadge =
        ...
        <div class="bg-dark-secondary rounded-lg p-4 border border-gray-700 hover:border-gray-600 transition-all">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              ...
              ${
                notif.method
                  ? `<span class="inline-block mt-2 px-2 py-1 text-xs font-semibold bg-blue-900 text-blue-300 rounded">${notif.method}</span>`
                  : ""
              }
            </div>
  ...
  async deleteNotification(notificationId) {
    console.log(
      `[${this.tabId}] Attempting to delete notification via API:`,
      notificationId
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `[${this.tabId}] Notification deleted successfully:`,
          notificationId,
          result.message
        );
        this.render();
        ...
        console.error(
          `[${this.tabId}] API Error deleting notification ${notificationId}:`,
          result.error || "Unknown server error"
        );
      }
    } catch (error) {
      console.error(
        `[${this.tabId}] Network Error deleting notification ${notificationId}:`,
        error
      );
    }
  }
...
window.notify = {
  success(message, action = null) {
    window.notificationSystem?.add("success", message, action);
  },
  error(message, action = null) {
    window.notificationSystem?.add("error", message, action);
  },
  warning(message, action = null) {
    window.notificationSystem?.add("warning", message, action);
  },
  info(message, action = null) {
    window.notificationSystem?.add("info", message, action);
  },
  clearAll() {
    window.notificationSystem?.clearAll();
  },
  sync() {
    window.notificationSystem?.syncWithBackend();
  },
};
